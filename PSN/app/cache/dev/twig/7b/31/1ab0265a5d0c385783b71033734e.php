<?php

/* WebProfilerBundle:Collector:config.html.twig */
class __TwigTemplate_7b311ab0265a5d0c385783b71033734e extends Twig_Template
{
    protected $parent;

    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->blocks = array(
            'toolbar' => array($this, 'block_toolbar'),
            'menu' => array($this, 'block_menu'),
            'panel' => array($this, 'block_panel'),
        );
    }

    public function getParent(array $context)
    {
        if (null === $this->parent) {
            $this->parent = $this->env->loadTemplate("WebProfilerBundle:Profiler:layout.html.twig");
        }

        return $this->parent;
    }

    public function display(array $context, array $blocks = array())
    {
        $context = array_merge($this->env->getGlobals(), $context);

        $this->getParent($context)->display($context, array_merge($this->blocks, $blocks));
    }

    // line 3
    public function block_toolbar($context, array $blocks = array())
    {
        // line 4
        echo "    ";
        ob_start();
        // line 5
        echo "        <a href=\"http://symfony.com/\"><img width=\"26\" height=\"28\" alt=\"Symfony\" style=\"border-width: 0; margin: 0 5px 0 10px; vertical-align: middle;\" src=\"data:image/png;base64,\"/></a>
    ";
        $context['icon'] = new Twig_Markup(ob_get_clean());
        // line 7
        echo "    ";
        $template = "WebProfilerBundle:Profiler:toolbar_item.html.twig";
        if ($template instanceof Twig_Template) {
            $template->display(array_merge($context, array("link" => false, "text" => $this->getAttribute($this->getContext($context, 'collector', '7'), "symfonyversion", array(), "any", false, 7))));
        } else {
            echo $this->env->getExtension('templating')->getTemplating()->render($template, array_merge($context, array("link" => false, "text" => $this->getAttribute($this->getContext($context, 'collector', '7'), "symfonyversion", array(), "any", false, 7))));
        }
        // line 8
        echo "
    ";
        // line 9
        if ($this->getContext($context, 'verbose', '9')) {
            // line 10
            echo "        ";
            ob_start();
            // line 11
            echo "            ";
            ob_start();
            // line 12
            echo "                <span>PHP ";
            echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '12'), "phpversion", array(), "any", false, 12), "html");
            echo "</span>
                <span style=\"margin: 0; padding: 0; color: #979696;\">|</span>
                <span style=\"color: ";
            // line 14
            echo (($this->getAttribute($this->getContext($context, 'collector', '14'), "hasxdebug", array(), "any", false, 14)) ? ("#759e1a") : ("#a33"));
            echo "\">xdebug</span>
                <span style=\"margin: 0; padding: 0; color: #979696\">|</span>
                <span style=\"color: ";
            // line 16
            echo (($this->getAttribute($this->getContext($context, 'collector', '16'), "hasaccelerator", array(), "any", false, 16)) ? ("#759e1a") : ("#a33"));
            echo "\">accel</span>
            ";
            echo trim(preg_replace('/>\s+</', '><', ob_get_clean()));
            // line 18
            echo "        ";
            $context['text'] = new Twig_Markup(ob_get_clean());
            // line 19
            echo "        ";
            $template = "WebProfilerBundle:Profiler:toolbar_item.html.twig";
            if ($template instanceof Twig_Template) {
                $template->display(array_merge($context, array("link" => false, "icon" => "")));
            } else {
                echo $this->env->getExtension('templating')->getTemplating()->render($template, array_merge($context, array("link" => false, "icon" => "")));
            }
            // line 20
            echo "    ";
        }
        // line 21
        echo "
    ";
        // line 22
        ob_start();
        // line 23
        echo "        <img width=\"21\" height=\"28\" alt=\"Environment\" style=\"border-width: 0; vertical-align: middle; margin-right: 5px;\" src=\"data:image/png;base64,
        \"/>
    ";
        $context['icon'] = new Twig_Markup(ob_get_clean());
        // line 26
        echo "    ";
        ob_start();
        // line 27
        echo "        ";
        ob_start();
        // line 28
        echo "            ";
        if ($this->getContext($context, 'verbose', '28')) {
            // line 29
            echo "                <span>";
            echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '29'), "appname", array(), "any", false, 29), "html");
            echo "</span>
                <span style=\"margin: 0; padding: 0; color: #979696;\">|</span>
                <span>";
            // line 31
            echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '31'), "env", array(), "any", false, 31), "html");
            echo "</span>
                <span style=\"margin: 0; padding: 0; color: #979696;\">|</span>
                <span>";
            // line 33
            echo (($this->getAttribute($this->getContext($context, 'collector', '33'), "debug", array(), "any", false, 33)) ? ("debug") : ("no-debug"));
            echo "</span>
                <span style=\"margin: 0; padding: 0; color: #979696;\">|</span>
            ";
        }
        // line 36
        echo "            <span>
                ";
        // line 37
        if ($this->getContext($context, 'profiler_url', '37')) {
            // line 38
            echo "                    <a style=\"color: #2f2f2f\" href=\"";
            echo twig_escape_filter($this->env, $this->getContext($context, 'profiler_url', '38'), "html");
            echo "\">";
            echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '38'), "token", array(), "any", false, 38), "html");
            echo "</a>
                ";
        } else {
            // line 40
            echo "                    ";
            echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '40'), "token", array(), "any", false, 40), "html");
            echo "
                ";
        }
        // line 42
        echo "            </span>
        ";
        echo trim(preg_replace('/>\s+</', '><', ob_get_clean()));
        // line 44
        echo "    ";
        $context['text'] = new Twig_Markup(ob_get_clean());
        // line 45
        echo "    ";
        $template = "WebProfilerBundle:Profiler:toolbar_item.html.twig";
        if ($template instanceof Twig_Template) {
            $template->display(array_merge($context, array("link" => $this->getContext($context, 'profiler_url', '45'))));
        } else {
            echo $this->env->getExtension('templating')->getTemplating()->render($template, array_merge($context, array("link" => $this->getContext($context, 'profiler_url', '45'))));
        }
    }

    // line 48
    public function block_menu($context, array $blocks = array())
    {
        // line 49
        echo "<span class=\"label\">
    <span class=\"icon\"><img src=\"";
        // line 50
        echo twig_escape_filter($this->env, $this->env->getExtension('templating')->getAssetUrl("bundles/webprofiler/images/profiler/config.png"), "html");
        echo "\" alt=\"Configuration\" /></span>
    <strong>Config</strong>
</span>
";
    }

    // line 55
    public function block_panel($context, array $blocks = array())
    {
        // line 56
        echo "    <h2>Project Configuration</h2>
    <table>
        <tr>
            <th>Key</th>
            <th>Value</th>
        </tr>
        <tr>
            <th>Symfony version</th>
            <td>";
        // line 64
        echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '64'), "symfonyversion", array(), "any", false, 64), "html");
        echo "</td>
        </tr>
        <tr>
            <th>Application name</th>
            <td>";
        // line 68
        echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '68'), "appname", array(), "any", false, 68), "html");
        echo "</td>
        </tr>
        <tr>
            <th>Environment</th>
            <td>";
        // line 72
        echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '72'), "env", array(), "any", false, 72), "html");
        echo "</td>
        </tr>
        <tr>
            <th>Debug</th>
            <td>";
        // line 76
        echo (($this->getAttribute($this->getContext($context, 'collector', '76'), "debug", array(), "any", false, 76)) ? ("enabled") : ("disabled"));
        echo "</td>
        </tr>
    </table>

    <h2>PHP configuration</h2>
    <table>
        <tr>
            <th>Key</th>
            <th>Value</th>
        </tr>
        <tr>
            <th>PHP version</th>
            <td>";
        // line 88
        echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '88'), "phpversion", array(), "any", false, 88), "html");
        echo "</td>
        </tr>
        <tr>
            <th>Xdebug</th>
            <td>";
        // line 92
        echo (($this->getAttribute($this->getContext($context, 'collector', '92'), "hasxdebug", array(), "any", false, 92)) ? ("enabled") : ("disabled"));
        echo "</td>
        </tr>
        <tr>
            <th>PHP acceleration</th>
            <td>";
        // line 96
        echo (($this->getAttribute($this->getContext($context, 'collector', '96'), "hasaccelerator", array(), "any", false, 96)) ? ("enabled") : ("disabled"));
        echo "</td>
        </tr>
        <tr>
            <th>XCache</th>
            <td>";
        // line 100
        echo (($this->getAttribute($this->getContext($context, 'collector', '100'), "hasxcache", array(), "any", false, 100)) ? ("enabled") : ("disabled"));
        echo "</td>
        </tr>
        <tr>
            <th>APC</th>
            <td>";
        // line 104
        echo (($this->getAttribute($this->getContext($context, 'collector', '104'), "hasapc", array(), "any", false, 104)) ? ("enabled") : ("disabled"));
        echo "</td>
        </tr>
        <tr>
            <th>EAccelerator</th>
            <td>";
        // line 108
        echo (($this->getAttribute($this->getContext($context, 'collector', '108'), "haseaccelerator", array(), "any", false, 108)) ? ("enabled") : ("disabled"));
        echo "</td>
        </tr>
    </table>

    <h2>Active bundles</h2>
    <table>
        <tr>
            <th>Name</th>
            <th>Path</th>
        </tr>
        ";
        // line 118
        $context['_parent'] = (array) $context;
        $context['_seq'] = twig_ensure_traversable($this->getAttribute($this->getContext($context, 'collector', '118'), "bundles", array(), "any", false, 118));
        foreach ($context['_seq'] as $context['name'] => $context['path']) {
            // line 119
            echo "        <tr>
            <th>";
            // line 120
            echo twig_escape_filter($this->env, $this->getContext($context, 'name', '120'), "html");
            echo "</th>
            <td>";
            // line 121
            echo twig_escape_filter($this->env, $this->getContext($context, 'path', '121'), "html");
            echo "</td>
        </tr>
        ";
        }
        $_parent = $context['_parent'];
        unset($context['_seq'], $context['_iterated'], $context['name'], $context['path'], $context['_parent'], $context['loop']);
        $context = array_merge($_parent, array_intersect_key($context, $_parent));
        // line 124
        echo "    </table>

";
    }

    public function getTemplateName()
    {
        return "WebProfilerBundle:Collector:config.html.twig";
    }
}
